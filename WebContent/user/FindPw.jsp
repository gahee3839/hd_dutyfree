...
$(function(){
    	...
    	/* $.ajax({
            ...
            success : function(data, textStatus, jqXHR) {
                if(crew.ajaxValidate(data, false)){
                    ...
                    location.href = ctx_curr + '/mm/mbshAuca/membershipFindPwdAuth.do';
                } else {
                	$('.t_error', '#divId').show().text('아이디를 다시 확인해주세요.');
                }
            },
            error : function(jqXHR, textStatus, errorThrown) {
    ...
    // 엔터키처리
    $('#mbshId', '#frmMbshFindPwd').keydown(function(e) {
        if(e.keyCode == 13) {
        	$('#btnConfirm', '#frmMbshFindPwd').click();
        	e.preventDefault();
        }
    });
});
...
function openDRMOKWindow(type,goodsCd,qty,buynow,setGoosId, cartSeq , cartSetId, notAdtGoodsCd , notAdtQtyList){
    ...
    window.name = 'sendJsp';
    DRMOK_window = window.open(url+'?cpid=hddfs&rtn_url=http://www.hddfs.com/shop/mm/mbshAuca/adultAuca.do?goosInfo='+goosInfo+'&req_info=Zpf7TPvrzLEqt6kfMQsyXhTTpyWozmn%2FApI8kFdEu429vgyB2olHXCvTJoyGMD6x9%2FPomla1BrS5MF%2FgDrmV27uSYjBk02FKUeIUj5ch65kmYnuJAhO8ECdh1cxfTiZwfAw5OYTjv9K%2FXUSedZl%2B6DEIuC8ib%2F12y%2BCHlUWoD6axrcHIhtGT64M9O9qt7C4xnde7jseUTH8NS6Vhh%2BhjuIV1nmtFNbSL5vWaLQUqnv5VwxWpghO9cN2wPXZnagztCBtyt2OjG%2F4aec5EG1e%2F4XsjDtlfrc3WZ7UotuI%2BsA7zhOljgqpXeDlyUvza5NSNMQfJJcw%2FkNR2m680IMyIUQ%3D%3D', 'DRMOKWindow', 'width=425,height=550,scrollbars=no,toolbar=no,location=no,directories=no,status=no' );
    DRMOK_window.focus();
