...
<body>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$('#memId, #memPwd', '#memLgin').keypress(
					function(e) {
						var s = String.fromCharCode(e.which);
						if (s.toUpperCase() === s && s.toLowerCase() !== s
								&& !e.shiftKey) {
							$('#pError', '#memLgin').show().text(
									"<Caps Lock>이 켜져 있습니다.");
						} else {
							$('#pError', '#memLgin').hide();
						}
			...
			$('#btnLgin', '#memLgin').click(
					function() {
						// trim 처리
						$('#memId', '#memLgin').val(
								$.trim($('#memId', '#memLgin').val()));
						$('#memPw', '#memLgin').val(
								$.trim($('#memPw', '#memLgin').val()));
		...
		function lginUmbMbsh(vInfo) {
			if (vInfo.code == '0000') { // 성공
				...
				if (vInfo.succYn == 'Y') {
					...
					gfnReqSSoToknIssuHttp(vInfo.ssoAuthMcustNo,
							vInfo.ssoAuthCd, fnSsoToknCallback);
				}
				...
				if (vInfo.succYn == 'Y') {
					...
					gfnReqSSoToknIssuHttp(vInfo.ssoAuthMcustNo,
							vInfo.ssoAuthCd, fnSsoToknPwdChgCallback);
				}
		...
		function fnUmbSimpJoinPop(ssoMcustNo, joinType) {
			...
			$form
					.append('<input type="hidden" value="' + ssoMcustNo + '" name="ssoMcustNo">');
			$form
					.append('<input type="hidden" value="PC" name="ptcoReqnMdaInf">');
			$form
	...
	<div class="pop_wrap">
		...
		<div class="pop_content">
			...
			<form id="memLgin" name="memLgin" method="post" action="DutyfreeServlet?command=login" autocomplete="off">
				<input type="hidden" name="aucaType" value="GENE" /> <input type="hidden" name="authType" value="GENE" />
				<!-- 탭구분용 -->
				<div class="join_form">
					<div class="join_row">
						<!-- 0914 박진수 추가  -->
				...
				<div class="id_save">
					<span class="checkbox small"> <input type="checkbox" name="saveId" id="saveId" value="Y"> <label for="saveId">아이디 저장</label>
					</span> <span class="find_idpw"> <!-- 0914 박진수 추가  --> <a href="DutyfreeServlet?command=FindId_Page">아이디찾기</a> <a href="DutyfreeServlet?command=FindPw_Page">비밀번호찾기</a> <!-- -->
					</span>
				</div>
				<div class="btn_login">
					<button type="button" class="btn_basic2 big" id="btnLgin">로그인</button>
				</div>
			</form>
		...
		<div class="txt_btn tl">
			<p>
				회원가입 시 더 많은 혜택이 제공됩니다.
				<a href="javascript:fnOpenerLocationClose('/mm/mbshJoin/join.do');" class="btn_basic1">회원가입</a>
				
			</p>
			<!-- 0914 진수 수정 -->
			<p>
				<a href="DutyfreeServlet?command=index" class="btn_basic2">메인으로</a>
		...
		<div class="fix_btn two">
			<ul>
				<li><a href="javascript:fnOpenerLocationClose('/or/order/listCart.do');" class="ico_coma i_link">비회원 주문</a></li>
				<li><a href="https://www.hddfs.com/shop/mm/myOrder/searchNmbshOrder.do" class="ico_coma i_link">비회원 주문 조회</a></li>
			</ul>
		</div>
	</div>

	<!-- S : dialog -->
	<div id="pw_error" title="로그인 안내">
		<div class="layer_popup">
			<div class="message_box">
				<!-- S : 2017-08-23 modify -->
				<p class="small_txt">
					동일 아이디로 비밀번호 5회 오류가 되어<br>로그인 제한되었습니다.
				</p>
				<!-- E : 2017-08-23 modify -->
			</div>
			<div class="basic_btn_box">
				<a href="javascript:fnOpenerLocationClose('/mm/mbshAuca/membershipFindId.do');" class="btn_basic1">아이디 찾기</a> <a href="javascript:fnOpenerLocationClose('/mm/mbshAuca/membershipFindPwd.do');" class="btn_basic1">비밀번호 찾기</a>
			</div>
		</div>
	</div>

	<div id="mem_error" title="로그인 안내">
		<input type="hidden" id="ssoMcustNo" name="ssoMcustNo" value="">
		<div class="layer_popup">
			<div class="message_box">
				<!-- S : 2017-08-23 modify -->
				<p class="small_txt">
					현대백화점인터넷면세점에 가입되지 않은<br>H.Point통합회원입니다.<br>H.Point 통합멤버십 페이지에서<br>현대백화점인터넷면세점 회원가입 하실 수 있습니다.
				</p>
				<!-- E : 2017-08-23 modify -->
			</div>
			<div class="basic_btn_box">
	...
	<div id="pw_noti" title="비밀번호 변경" style="display: none;">
		<div class="layer_popup">
			<div class="message_box">
				<p class="small_txt">
					통합멤버십 비밀번호 변경 대상 회원입니다.<br>비밀번호를 변경해주세요
				</p>
			</div>
	...
	<script>
		...
		function loginCallback(type, data) {
			var redirectUrl = "https://www.hddfs.com/shop/dm/main.do".replace(
					/&amp;/gi, '&');
			if (redirectUrl.indexOf("token") != -1) {
				redirectUrl = redirectUrl.slice(0, redirectUrl
						.indexOf("&token"));
			}
			var orderYn = "";
			...
			if (crew.ajaxValidate(data, false)) { // 로그인 성공
				...
				if (notAdtGoodsCd != null && notAdtGoodsCd != ""
						&& notAdtGoodsCd != "undefined") {
					setCookie('NOT_ADULT_GOOSCD', notAdtGoodsCd);
				}

				if (notAdtQtyList != null && notAdtQtyList != ""
						&& notAdtQtyList != "undefined") {
					setCookie('NOT_ADULT_QTY', notAdtQtyList);
				}

				if (setGoosId != null && setGoosId != ""
						&& setGoosId != "undefined") {
				...
				if (orderYn == "Y") {
					...
					if (redirectUrl != "") {
						...
						var rtnVal = false;
						var arrJoinUrl = [ "/mm/mbshJoin/join.do",
								"/mm/mbshJoin/termsAgree.do",
								"/mm/mbshJoin/authentication.do",
								"/mm/mbshJoin/mbshInformation",
								"/mm/mbshJoin/joinComplete.do" ];
				...
				if (vResultCode == 100) { // 계정잠금
					...
					opener.fnMbshSleepRelease(vInfo.mbshNo, vInfo.authType,
							redirectUrl);
				} else if (vResultCode == 120) { // 임시 비밀번호 변경 안내
					alert(vMessage);

					opener.location.href = ctx_curr
							+ '/mm/myInfo/inptMbshPwd.do?type=pwd';
				} else if (vResultCode == 130) { // 비밀번호 장기 미변경

					opener.location.href = ctx_curr
							+ '/mm/mbshAuca/membershipChangePassword.do?redirectUrl='
							+ redirectUrl;
					...
					location.href = ctx_curr
							+ '/mm/mbshAuca/addLginBlank.do?resultCode='
							+ vResultCode + '&redirectUrl=' + redirectUrl;
					return;
		...
		function kakaoLginUmbMbsh() {
			...
			var ret = window.open('', 'hpKkoLogin', "width=430,height=650",
					false);
			frm.submit();
		}
